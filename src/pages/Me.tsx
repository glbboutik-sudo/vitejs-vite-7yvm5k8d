...
export default function Me({ goTab }: { goTab: (t: string) => void }) {
  ...
  function togglePrivacy() {
    ...
    const next = opts[(opts.indexOf(privacy)+1) % 3]
    updatePrivacy(next)
  }
  ...
  return (
    <div style={{ paddingBottom:80 }}>
      ...
      <div style={{ padding:'30px 14px 10px', background:'#fafafa' }}>
        <div style={{ fontSize:15, fontWeight:800, color:'#111' }}>{profile?.full_name || user?.email || 'Mon profil'}</div>
      ...
      <div style={{ display:'flex', background:'#fff', border:'0.5px solid #f0eded', borderRadius:12, margin:'10px 14px', overflow:'hidden' }}>
        {[
          ...
          [String(stats.vues), 'Vues'],
          [String(stats.favoris), 'Favoris'],
          ['⭐ -', 'Note'],
        ].map(([n,l]) => (
