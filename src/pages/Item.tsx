...
export default function Item({ item, goBack }: { item: any, goBack: () => void }) {
  ...
  useEffect(() => {
    ...
    // Vérifier si déjà en favori (localStorage)
  ...
  return (
    <div style={{ paddingBottom:80, background:'#fafafa', minHeight:'100vh' }}>
      ...
      <div style={{ background:'#fff', padding:'12px 14px', display:'flex', alignItems:'center', gap:10, borderBottom:'0.5px solid #f0eded', position:'sticky', top:0, zIndex:10 }}>
        ...
        {/* BOUTON FAVORI HEADER */}
        <button onClick={toggleFavorite} style={{ width:32, height:32, borderRadius:'50%', background: liked ? '#FDEDEC' : '#f5f5f5', border:'none', cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s' }}>
          {liked ? '❤️' : '🤍'}
        </button>
      </div>
      ...
      <div style={{ height:260, background:'#FDEDEC', display:'flex', alignItems:'center', justifyContent:'center', fontSize:80, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', bottom:12, left:12, display:'flex', gap:6 }}>
          <div style={{ background:'rgba(0,0,0,.5)', borderRadius:20, padding:'4px 10px', fontSize:10, fontWeight:600, color:'#fff', display:'flex', alignItems:'center', gap:4 }}>
            👁 {(item.views || 0) + 1}
          </div>
          <div style={{ background:'rgba(0,0,0,.5)', borderRadius:20, padding:'4px 10px', fontSize:10, fontWeight:600, color:'#fff', display:'flex', alignItems:'center', gap:4 }}>
            ❤️ {favorites}
